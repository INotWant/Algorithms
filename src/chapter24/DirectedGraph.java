...
/**
 * 有向图数据结构（类同 @{@link chapter23.UndirectedGraph}）
 *
 * @author kissx on 2017/12/5.
 */
public class DirectedGraph {

    private Map<String, ENode> eMap = new HashMap<>();  // 边集（邻接链表）
    private String[] vArray;    // 点集
    private List<Edge> eArray = new ArrayList<>();  // 边集（原始）
    ...
    public DirectedGraph(String[] vArray, Edge[] eArray) {
        this.vArray = vArray;
        this.eArray.addAll(Arrays.asList(eArray));
        // 构建邻接链表
        for (Edge edge : eArray) {
            ENode ENode = eMap.get(edge.v1);
            init(ENode, edge.v1, edge.v2, edge.weight);
        }
    }
